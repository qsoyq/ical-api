...
router = APIRouter(tags=["iCalendar"], prefix="/ics/gofans")
...
@router.get("/iOS", summary="iOS 限免限免应用", include_in_schema=False)
@router.get("/iOS.ics", summary="iOS 限免应用")
async def ios(
    limit: int = Query(8),
    page: int = Query(1),
):
...
@router.get("/macOS", summary="macOS 限免应用", include_in_schema=False)
@router.get("/macOS.ics", summary="macOS 限免应用")
async def macOS(
    limit: int = Query(8),
    page: int = Query(1),
):
